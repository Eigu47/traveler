...
export default function ResultCard({
  ...
  return (
    <article
      className={`m-2 flex cursor-default flex-col rounded-xl bg-slate-100 text-center text-sm shadow ring-1 ring-black/20 duration-75 ease-out sm:text-base ${
      ...
      ref={resultRef}
    >
      <div className="flex h-36 rounded-xl border-b border-black/10 bg-slate-200 sm:h-44">
        ...
        <div className="flex w-full flex-col px-3 sm:pt-2">
          ...
          {place.rating && (
            <div className="flex items-center justify-center space-x-1.5 sm:py-1.5">
              <span>{place.rating}</span>
              <Rating
                rating={place.rating}
                className="justify-center text-base sm:text-xl"
              />
              <span className="whitespace-nowrap sm:whitespace-normal">{`${place.user_ratings_total} rewiews`}</span>
            </div>
          )}
        </div>
      </div>
      <div className="w-full space-y-2 p-2 pt-4 sm:space-y-4">
